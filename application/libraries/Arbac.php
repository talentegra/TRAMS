...
/**
 * This Library class is forked from AAuth	
 * ARBAC is a User Authorization Library for CodeIgniter 2.x, which aims to make 
 ...
 *
 * The latest version of ARBAC can be obtained from:
 * https://github.com/talentegra/CodeIgniter-ARBAC
 *
...
class Arbac {
	...
	public function login($identifier, $pass, $remember = FALSE, $totp_code = NULL) {
		...
		if($this->config_vars['use_cookies'] == TRUE){
 		...
 		if( $this->config_vars['login_with_username'] == TRUE){
		...
		*/
		...
		$query = $this->arbac_db->where('pass', $this->hash_password($pass, $user_id));
	...
	public function control( $perm_par = FALSE ){
		...
		if ( ! $this->is_allowed($perm_id) OR ! $this->is_group_allowed($perm_id) ){
			if( $this->config_vars['no_permission'] ) {
				$this->error($this->CI->lang->line('arbac_error_no_access'));
				redirect($this->config_vars['no_permission']);
			}
			else {
				echo $this->CI->lang->line('arbac_error_no_access');
				die();
			}
		}
	}
	...
	public function login_fast($user_id){
		...
		$query = $this->arbac_db->where('id', $user_id);
		$query = $this->arbac_db->where('banned', 0);
		$query = $this->arbac_db->get($this->config_vars['users']);
	...
	public	function reset_login_attempts($user_id) {
		...
		$this->arbac_db->where('id', $user_id);
	...
	public function remind_password($email){
		...
		if ($query->num_rows() > 0){
			...
			$this->arbac_db->where('email', $email);
			...
			$this->CI->email->subject($this->CI->lang->line('arbac_email_reset_subject'));
			$this->CI->email->message($this->CI->lang->line('arbac_email_reset_text') . site_url() . $this->config_vars['reset_password_link'] . $row->id . '/' . $ver_code );
			$this->CI->email->send();
	...
	public function reset_password($user_id, $ver_code){
		...
		$query = $this->arbac_db->where('id', $user_id);
		$query = $this->arbac_db->where('verification_code', $ver_code);
		$query = $this->arbac_db->get( $this->config_vars['users'] );
		...
		if( $query->num_rows() > 0 ){
			...
			$this->CI->email->subject($this->CI->lang->line('arbac_email_reset_success_subject'));
			$this->CI->email->message($this->CI->lang->line('arbac_email_reset_success_new_password') . $pass);
			$this->CI->email->send();
	...
	public function create_user($email, $pass, $username = FALSE) {
		...
		if ( $this->arbac_db->insert($this->config_vars['users'], $data )){
			...
			// set default role 
			$this->set_primary_role($user_id, $this->config_vars['default_group']);
	...
	public function update_user($user_id, $email = FALSE, $pass = FALSE, $username = FALSE) {
		...
		if ($email != FALSE) {
			if ($this->user_exist_by_email($email)) {
				$this->error($this->CI->lang->line('arbac_error_update_email_exists'));
				$valid = FALSE;
			}
			$valid_email = (bool) filter_var($email, FILTER_VALIDATE_EMAIL);
			if (!$valid_email){
		...
		if ($username != FALSE) {
			if ($this->user_exist_by_username($username)) {
				$this->error($this->CI->lang->line('arbac_error_update_username_exists'));
				$valid = FALSE;
			}
			/*if ($username !='' && !ctype_alnum(str_replace($this->config_vars['valid_chars'], '', $username))){
				$this->error($this->CI->lang->line('arbac_error_username_invalid'));
				$valid = FALSE;
			}*/
	...
	public function list_users($group_par = FALSE, $limit = FALSE, $offset = FALSE, $include_banneds = FALSE) {
		...
		if ($limit) {
			...
			if ($offset == FALSE)
				$this->arbac_db->limit($limit);
			else
				$this->arbac_db->limit($limit, $offset);
		}

		$query = $this->arbac_db->get();
	...
	public function send_verification($user_id){
		...
		if ($query->num_rows() > 0){
			...
			$this->CI->email->from( $this->config_vars['email'], $this->config_vars['name']);
			$this->CI->email->to($row->email);
			$this->CI->email->subject($this->CI->lang->line('arbac_email_verification_subject'));
			$this->CI->email->message($this->CI->lang->line('arbac_email_verification_code') . $ver_code .
				$this->CI->lang->line('arbac_email_verification_text') . site_url() .$this->config_vars['verification_link'] . $user_id . '/' . $ver_code );
			$this->CI->email->send();
		}
	}
	...
	public function delete_user($user_id) {
		...
		$this->arbac_db->where('user_id', $user_id);
		$this->arbac_db->delete($this->config_vars['perm_to_user']);

		// delete from user_to_group
		$this->arbac_db->where('user_id', $user_id);
		$this->arbac_db->delete($this->config_vars['user_to_group']);

		// delete user vars
		$this->arbac_db->where('user_id', $user_id);
		$this->arbac_db->delete($this->config_vars['user_variables']);

		// delete user
		$this->arbac_db->where('id', $user_id);
		return $this->arbac_db->delete($this->config_vars['users']);
	...
	public function user_exist_by_email( $user_email ) {
		$query = $this->arbac_db->where('email', $user_email);
	...
	public function get_user_groups($user_id = FALSE){
		...
		$this->arbac_db->select('*');
		$this->arbac_db->from($this->config_vars['user_to_group']);
		$this->arbac_db->join($this->config_vars['groups'], "id = group_id");
		$this->arbac_db->where('user_id', $user_id);

		return $query = $this->arbac_db->get()->result();
	}
	...
	public function create_group($group_name, $definition = '') {
		...
		$query = $this->arbac_db->get_where($this->config_vars['groups'], array('name' => $group_name));
		...
		if ($query->num_rows() < 1) {
			...
			$this->arbac_db->insert($this->config_vars['groups'], $data);
			return $this->arbac_db->insert_id();
		}

		$this->info($this->CI->lang->line('arbac_info_group_exists'));
		return FALSE;
	}
	...
	public function update_group($group_par, $group_name=FALSE, $definition=FALSE) {
		...
		$this->arbac_db->where('id', $group_id);
		return $this->arbac_db->update($this->config_vars['groups'], $data);
	}
	...
	public function delete_group($group_par) {
		...
		$this->arbac_db->where('id',$group_id);
		$query = $this->arbac_db->get($this->config_vars['groups']);
		if ($query->num_rows() == 0){
			return FALSE;
		}
		...
		$this->arbac_db->where('group_id', $group_id);
		$this->arbac_db->delete($this->config_vars['user_to_group']);
		
		$this->arbac_db->where('group_id', $group_id);
		$this->arbac_db->delete($this->config_vars['perm_to_group']);

		$this->arbac_db->where('id', $group_id);
		return $this->arbac_db->delete($this->config_vars['groups']);
	}
	...
	public function add_member($user_id, $group_par) {
		...
		if( ! $group_id ) {
			...
			$this->error( $this->CI->lang->line('arbac_error_no_group') );
			return FALSE;
		}

		$query = $this->arbac_db->where('user_id',$user_id);
		$query = $this->arbac_db->where('group_id',$group_id);
		$query = $this->arbac_db->get($this->config_vars['user_to_group']);
		...
		if ($query->num_rows() < 1) {
			...
			return $this->arbac_db->insert($this->config_vars['user_to_group'], $data);
		}
	 ...
	 * @param int $user_id User id to add to role
	 * @param int|string $group_par role id or name to add user to
	 * @return bool Add success/failure
	 */
	public function set_primary_role($user_id, $group_par) {
		...
		$query = $this->arbac_db->where('staff_id',$user_id);
		$query = $this->arbac_db->get($this->config_vars['staff']);
		if ($query->num_rows() < 1) {
			$data = array(
				'staff_id' => $user_id,
				'role_id' => $group_id
			...
			$query = $this->arbac_db->where('staff_id',$user_id);
			$query = $this->arbac_db->where('role_id',$group_id);
			$query = $this->arbac_db->get($this->config_vars['staff']);
			...
			$query = $this->arbac_db->where('staff_id',$user_id);
			//$query = $this->arbac_db->update($this->config_vars['staff'], $data);
			//return $query;
			return $this->arbac_db->update($this->config_vars['staff'], $data);
		}
		return TRUE;
		}
	...
	public function remove_member($user_id, $group_par) {
		...
		$group_par = $this->get_group_id($group_par);
		$this->arbac_db->where('user_id', $user_id);
		$this->arbac_db->where('group_id', $group_par);
		return $this->arbac_db->delete($this->config_vars['user_to_group']);
	}
	...
	public function get_group_name($group_id) {
		...
		$query = $this->arbac_db->where('id', $group_id);
		$query = $this->arbac_db->get($this->config_vars['groups']);
	...
	public function get_group_id ( $group_par ) {
		...
		$query = $this->arbac_db->where('name', $group_par);
		$query = $this->arbac_db->get($this->config_vars['groups']);
	...
	public function create_perm($perm_name, $definition='') {
		...
		$query = $this->arbac_db->get_where($this->config_vars['perms'], array('name' => $perm_name));
		...
		if ($query->num_rows() < 1) {
			...
			$this->arbac_db->insert($this->config_vars['perms'], $data);
			return $this->arbac_db->insert_id();
		}
		$this->info($this->CI->lang->line('arbac_info_perm_exists'));
		return FALSE;
	}
	...
	public function update_perm($perm_par, $perm_name=FALSE, $definition=FALSE) {
		...
		$this->arbac_db->where('id', $perm_id);
		return $this->arbac_db->update($this->config_vars['perms'], $data);
	}
	...
	public function delete_perm($perm_par) {
		...
		$this->arbac_db->where('perm_id', $perm_id);
		$this->arbac_db->delete($this->config_vars['perm_to_group']);
		...
		$this->arbac_db->where('id', $perm_id);
		return $this->arbac_db->delete($this->config_vars['perms']);
	}
	...
	public function is_allowed($perm_par, $user_id=FALSE){
		...
		$query = $this->arbac_db->where('perm_id', $perm_id);
		$query = $this->arbac_db->where('user_id', $user_id);
		$query = $this->arbac_db->get( $this->config_vars['perm_to_user'] );
	...
	public function is_group_allowed($perm_par, $group_par=FALSE){
		...
		if($group_par != FALSE){
			...
			$query = $this->arbac_db->where('perm_id', $perm_id);
			$query = $this->arbac_db->where('group_id', $group_par);
			$query = $this->arbac_db->get( $this->config_vars['perm_to_group'] );
	...
	public function allow_user($user_id, $perm_par) {
		...
		if ($query->num_rows() < 1) {
			...
			return $this->arbac_db->insert($this->config_vars['perm_to_user'], $data);
		}
		return TRUE;
	}
	...
	public function deny_user($user_id, $perm_par) {
		...
		return $this->arbac_db->delete($this->config_vars['perm_to_user']);
	}
	...
	public function allow_group($group_par, $perm_par) {
		...
		if ($query->num_rows() < 1) {
			...
			return $this->arbac_db->insert($this->config_vars['perm_to_group'], $data);
		}
	...
	public function deny_group($group_par, $perm_par) {
		...
		return $this->arbac_db->delete($this->config_vars['perm_to_group']);
	}
	...
	public function list_perms() {
		...
		$query = $this->arbac_db->get($this->config_vars['perms']);
		return $query->result();
	}
	...
	public function get_perm_id($perm_par) {
		...
		$query = $this->arbac_db->where('name', $perm_par);
		$query = $this->arbac_db->get($this->config_vars['perms']);
	...
	public function send_pm( $sender_id, $receiver_id, $title, $message ){
		...
		if ( !is_numeric($receiver_id) OR $sender_id == $receiver_id ){
			$this->error($this->CI->lang->line('arbac_error_self_pm'));
			return FALSE;
		}

		$query = $this->arbac_db->where('id', $receiver_id);
		$query = $this->arbac_db->where('banned', 0);
		...
		$query = $this->arbac_db->where('id', $sender_id);
		$query = $this->arbac_db->where('banned', 0);
		...
		return $query = $this->arbac_db->insert( $this->config_vars['pms'], $data );
	}
	...
	public function list_pms($limit=5, $offset=0, $receiver_id = FALSE, $sender_id=FALSE){
		...
		if ( $receiver_id != FALSE){
			$query = $this->arbac_db->where('receiver_id', $receiver_id);
		}

		if( $sender_id != FALSE ){
			$query = $this->arbac_db->where('sender_id', $sender_id);
		}

		$query = $this->arbac_db->order_by('id','DESC');
		$query = $this->arbac_db->get( $this->config_vars['pms'], $limit, $offset);
		return $query->result();
	}
	...
	public function get_pm($pm_id, $set_as_read = TRUE){
		...
		$query = $this->arbac_db->where('id', $pm_id);
		$query = $this->arbac_db->get( $this->config_vars['pms'] );

		if ($query->num_rows() < 1) {
			$this->error( $this->CI->lang->line('arbac_error_no_pm') );
			return FALSE;
		}
	...
	public function delete_pm($pm_id){
		...
		return $this->arbac_db->delete( $this->config_vars['pms'], array('id' => $pm_id) );
	}
	...
	public function count_unread_pms($receiver_id=FALSE){
		...
		$query = $this->arbac_db->where('receiver_id', $receiver_id);
		$query = $this->arbac_db->where('date_read', NULL);
		$query = $this->arbac_db->get( $this->config_vars['pms'] );
	...
	public function set_as_read_pm($pm_id){
		...
		$this->arbac_db->update( $this->config_vars['pms'], $data, "id = $pm_id");
	}
	...
	public function set_user_var( $key, $value, $user_id = FALSE ) {
		...
		// if var not set, set
		 if ($this->get_user_var($key,$user_id) ===FALSE) {
			...
			return $this->arbac_db->insert( $this->config_vars['user_variables'] , $data);
		}
		// if var already set, overwrite
			...
			$this->arbac_db->where( 'data_key', $key );
			$this->arbac_db->where( 'user_id', $user_id);

			return $this->arbac_db->update( $this->config_vars['user_variables'], $data);
		}
	}
	...
	public function unset_user_var( $key, $user_id = FALSE ) {
		...
		return $this->arbac_db->delete( $this->config_vars['user_variables'] );
	}
	...
	public function get_user_var( $key, $user_id = FALSE){
		...
		$query = $this->arbac_db->where('user_id', $user_id);
		$query = $this->arbac_db->where('data_key', $key);

		$query = $this->arbac_db->get( $this->config_vars['user_variables'] );
	...
	public function list_user_var_keys($user_id = FALSE){
		...
		$query = $this->arbac_db->select('data_key');
	...
	########################
	# Arbac System Variables
	########################

	//tested
	/**
	 * Set Arbac System Variable as key value
	...
	public function set_system_var( $key, $value ) {
		...
		if ($this->get_system_var($key) === FALSE) {
			...
			return $this->arbac_db->insert( $this->config_vars['system_variables'] , $data);
		...
		// if var already set, overwrite
			...
			$this->arbac_db->where( 'data_key', $key );
			return $this->arbac_db->update( $this->config_vars['system_variables'], $data);
		}
	...
	//tested
	/**
	 * Unset Arbac System Variable as key value
	 * @param string $key
	 * @return bool
	 */
	public function unset_system_var( $key	) {
		...
		return $this->arbac_db->delete( $this->config_vars['system_variables'] );
	}

	//tested
	/**
	 * Get Arbac System Variable by key
	 * Return string of variable value or FALSE
	 * @param string $key
	 * @return bool|string , FALSE if var is not set, the value of var if set
	 */
	public function get_system_var( $key ){
		...
		$query = $this->arbac_db->get( $this->config_vars['system_variables'] );
	...
	public function generate_unique_totp_secret(){
		...
		while (!$stop) {
			...
			$query = $this->arbac_db->where('totp_secret', $secret);
			$query = $this->arbac_db->get($this->config_vars['users']);
			if ($query->num_rows() == 0) {
				return $secret;
				$stop = true;
	 ...
	 * @return bool
	 */
	public function is_staff_allowed($perm_par, $user_id=FALSE,$arr_role_id){
		...
		$query = $this->arbac_db->where('perm_id', $perm_id);
		$query = $this->arbac_db->where_in('group_id', $arr_role_id);
		$query = $this->arbac_db->get( $this->config_vars['perm_to_group'] );
                //echo "<br> query ".$this->arbac_db->last_query();exit;
 ...
 * User and Arbac System Variables.
...
/* Location: ./application/libraries/Arbac.php */
