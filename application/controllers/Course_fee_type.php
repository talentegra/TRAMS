...
/* Modified by Vivek Raghunathan 2016-06-21 15:48:05 */
...
class Course_fee_type extends APP_Controller
{
    ...
    public function index()
    {
        $data = array(
            ...
            'sort_by' => 'DESC',
            'sort_column' => 'course_fee_type_id',
			'q' => '',
			'total_rows' => ''
        );
    ...
    public function list_data()
    {
        ...
        $config['div'] = 'Course_fee_type_list'; //parent div tag id
        $config['base_url'] = base_url(). 'Course_fee_type/list_data';
        $config['total_rows'] = $this->Course_fee_type_model->total_rows($q);
        $config['per_page'] = $this->perPage;

        $this->ajax_pagination->initialize($config);

        //get the posts data
        $course_fee_type_data = $this->Course_fee_type_model->get_limit_data($config['per_page'], $offset,$sort_column ,$sort_by , $q);

        $data = array(
            'course_fee_type_data' => $course_fee_type_data,
            'page' => $page,
            'sort_by' => $sort_by,
        ...
        //load the view
        $this->load->view('course_fee_type/course_fee_type_list_data', $data);
    }
